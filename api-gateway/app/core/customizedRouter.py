...
from app.core.network import make_request
...
def route(
        ...
        authentication_token_decoder: str = 'app.core.auth.decode_access_token',
        service_authorization_checker: str = 'app.core.auth.is_admin_user',
        service_header_generator: str = 'app.core.auth.generate_request_header',
    ...
    def wrapper(f):
        ...
        async def inner(request: Request, response: Response, **kwargs):
            ...
            # if isinstance(payload_obj, OAuth2PasswordRequestForm):
            #     payload = {
            ...
            # If the response is expected (success), proceed
            ...
            # If the response is NOT expected (e.g. validation error), raise an HTTPException
            raise HTTPException(
                status_code=status_code_from_service,
                detail=resp_data.get("detail", "Unexpected error"),
            )
